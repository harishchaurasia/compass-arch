...
The aggregator is hand-tuned on a small dev split (~5 tasks) and locked before evaluation. The methodology is reported honestly in the writeup. A learned calibration probe (small classifier on these features) is the Phase 4 stretch goal - and FINDINGS.md §10-11 now carry it from motivation to a shipped, flagged implementation. §10: the cheaper structural alternative (precondition checks - target/destination grounding, read-before-write, in `analysis/discrimination.py`) scores at chance, because the compound-failure mode is a semantically wrong action on a correctly-grounded target. §11: a logistic probe over sentence-embedding action-vs-request affinities recovers the signal (held-out AUC 0.63 retail / 0.66 airline, transfers cross-domain), and is now wired behind `build_compass_agent(calibration="probe")` (`compass/probe.py`, weights from `scripts/fit_probe.py`) with the rule-based aggregator staying the locked default and a graceful fallback when no embedding backend is present.
