...
2. `uv run python scripts/leaderboard.py` to regenerate the board and its snapshot.
3. Open a PR with the updated `LEADERBOARD.md` **and** `results/leaderboard_data.json`
   (CI checks they stay in sync). The raw `results/trials.db` stays on your machine
   (it is gitignored); only the distilled aggregates are committed.
